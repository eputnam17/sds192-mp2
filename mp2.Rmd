...
#No - because cancellations receive a unique tran_id. Looking at this dataset, there are many cases where I can't figure out which transaction matches with each cancellation, so it is not possible to filter out cancelled donations with the information we have. Instead, we will filter out only the negative donations  
