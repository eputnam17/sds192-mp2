...
ggplot(ContributionsToEach, aes(x = as.Date(transaction_dt, "%m%d%Y"), y = transaction_amt, color = party)) + 
  ...
  geom_point() +
  labs(title = "Large Contributions in the 2012 House Election", y = "transaction amount", x = "transaction date") +
scale_x_date()
```
...
ggplot(BigContributions2012, aes(x = as.Date(transaction_dt, "%m%d%Y"), y = transaction_amt, label = name, color = party)) +
  ...
  geom_point(size = .5) +
  labs(title = "Top 50 House Election Contributions of 2012", x = "month", y = "transaction amount") +
...
ggplot(BigContributions2012, aes(x = transaction_dt, y = transaction_amt, label = name, color = party)) +
  ...
  geom_text(size = 2) +
  geom_point(size = .5) +
  labs(title = "Top 50 Contributions to the 2012 House Election", x = "transaction date", y = "transaction amount") +
...
Cands <- unique(listOfCand)

# here's the start of my function but idk where to go from here.
# my GOAL are to 1) confirm it was a sucessful list 2) sum up the money per candidate using the list
...
# the numeric value that these function calls produce is the total $ that was given to each house candidate
MoneyPerCandidate("H0AL02087")
MoneyPerCandidate("H2GA09143")
